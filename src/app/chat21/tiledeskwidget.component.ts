...
@Component({
    ...
    templateUrl: './tiledeskwidget.component.html',
    styleUrls: ['./tiledeskwidget.component.scss'],
...
export class TileDeskWidgetComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    */
     ...
     */
    startNwConversation() {
        this.g.wdLog(['TileDeskWidgetComponent::startNwConversation']);
 ...
 // #region OpenConversationByUser
 @ViewChild(HomeComponent) _homeComponent;
 openConversationByUser(user:any){//UserDetailModel
     //debugger;
     //this.listConversations=this._conversationsService.listConversations;
     let conversation;
    //  for(let i=0;i<this.listConversations.length;i++){
    //    if(user.id!=this.currentUser.uid){
    //     if(user.id === this.listConversations[i].recipient || user.id === this.listConversations[i].sender){
    //       this.listConversations[i].recipient=user.id; 
    //       this.listConversations[i].recipient_fullname=user.name;
    //       conversation=this.listConversations[i];
    //       break;
    //     }
    //    }  
    //  }
     //debugger;
     this.openCloseWidget(true,true);
     if(conversation!=null && conversation!=undefined && conversation!=''){
         this._homeComponent.returnSelectedConversation(conversation);
     }else{
         this._homeComponent.returnNewConversation(user);
     }
 }
 // #endregion
     ...
     */
    openCloseWidget($event,outSideCall:any) {
        if( outSideCall === true ){
            this.g.isOpen = true;
        }
        this.g.setParameter('displayEyeCatcherCard', 'none');
