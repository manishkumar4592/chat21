...
import { TileDeskWidgetComponent } from '../../tiledeskwidget.component';
...
export class ConversationComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  constructor(
    ...
    public tiledeskWidgetComponent: TileDeskWidgetComponent,
  ...
  ngOnInit() {
    ...
    const that = this;
    const subscriptionEndRenderMessage = this.tiledeskWidgetComponent.obsEndRenderMessage.subscribe(() => {
      this.ngZone.run(() => {
        that.scrollToBottom();
      });
    });
...
/**
  ...
  private restoreTextArea() {
    ...
    if (textArea) {
        ...
        this.g.wdLog(['TileDeskWidgetComponent:restoreTextArea::restoreTextArea::textArea:', 'restored']);
    } else {
          console.error('TileDeskWidgetComponent:restoreTextArea::restoreTextArea::textArea:', 'not restored');
    }
    this.setFocusOnId('chat21-main-message-context');
  }
  ...
  onSendPressed(event) {
    ...
    this.g.wdLog(['TileDeskWidgetComponent::onSendPressed::isFilePendingToUpload:', this.isFilePendingToUpload]);
    if (this.isFilePendingToUpload) {
      this.g.wdLog(['TileDeskWidgetComponent::onSendPressed', 'is a file']);
      // its a file
      this.loadFile();
      ...
      if ( this.textInputTextArea.length > 0 ) {
        this.g.wdLog(['TileDeskWidgetComponent::onSendPressed', 'is a message']);
        // its a message
        this.performSendingMessage();
...
// ========= begin:: functions send image ======= //
  ...
  detectFiles(event) {
    ...
    if (event) {
        ...
        this.g.wdLog(['TileDeskWidgetComponent:detectFiles::selectedFiles', this.selectedFiles]);
        if (this.selectedFiles == null) {
          this.isFilePendingToUpload = false;
        } else {
          this.isFilePendingToUpload = true;
        }
        this.g.wdLog(['TileDeskWidgetComponent:detectFiles::selectedFiles::isFilePendingToUpload', this.isFilePendingToUpload]);
   ...
   */
    uploadSingle(metadata, file) {
        ...
        that.g.wdLog(['TileDeskWidgetComponent::uploadSingle::', metadata, file]);
        // const file = this.selectedFiles.item(0);
        ...
        uploadTask.then(snapshot => {
            ...
            .then(downloadURL => {
                  that.g.wdLog(['TileDeskWidgetComponent::uploadSingle:: downloadURL', downloadURL]);
            ...
            .catch(error => {
                ...
                console.error(`TileDeskWidgetComponent::uploadSingle:: Failed to upload file and get link - ${error}`);
            });
      // this.resetLoadImage();
